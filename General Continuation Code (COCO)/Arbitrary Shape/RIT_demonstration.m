...
% Define the function as ode_RIT
...
for i = 1:length(BP)
    run_name = ['RIT_run' sprintf('%.0f',1+i)];
    prob = coco_prob();
...
for i = 1:length(UZ)
    run_name = ['RIT_run' sprintf('%.0f',1+i+length(BP))];
    prob = coco_prob();
...
for i = 1:length(RIT_runs)
    coco_plot_bd(theme1, RIT_runs(i).name, 'x',1,'x',2,'b')
    plot_shape_from_COCO(RIT_runs(i).name,N,N_modes,adjacency_matrix,points,coeff_matrix,modes_to_skip)
