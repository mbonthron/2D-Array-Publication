...
computational_domain = [0 0.15];         % What is the domain of b to explore
UZpoints = [0.10];                       % Values to explicitly call out to be used to plot

%% Continuation Constants
iterations_max = 5000;                  % Maximum number of iterations before continuation terminates
hmin = 0.00001;                           % Minimum step size of the continuation
hmax = 0.0001;                            % Maximum step size of the continuation
...
for i = 1:length(BP)
    run_name = ['triangle_run' sprintf('%.0f',1+i)];
    prob = coco_prob();
...
for i = 1:length(UZ)
    run_name = ['triangle_run' sprintf('%.0f',1+i+length(BP))];
    prob = coco_prob();
...
close all
cd('data');
triangle_runs = dir('triangle*');
cd ..

theme1 = struct('special', {{'EP','FP','HB','BP'}});
figure(9899); clf; hold on

for i = 1:length(triangle_runs)
    coco_plot_bd(theme1, triangle_runs(i).name, 'x',1,'x',2,'b')
    plot_shape_from_COCO(triangle_runs(i).name,N,N_modes,adjacency_matrix,points,coeff_matrix,modes_to_skip)
