...
%%
% load("Hinge Position.mat")
% hinge_radius = [50;50;50];
% hinge_position(2,:) = hinge_position(2,:) - (1-.999)*1e3*[1.2243    2.0769] ;
% hinge_position(3,:) = hinge_position(3,:) + (1-.999)*1e3*[1.2243    2.0769] ;
...
for i = 1:length(adjacency_matrix)
    % Plot the two hinges
    im = insertShape(im,'Circle',[hinge_position(i,:) hinge_radius(i,:)],'Color','red','linewidth',8);
end
...
for i = 1:3
    ...
    % Plot the x axis
    im = insertShape(im,'Line',[hinge_R  hinge_L],'Color','red','linewidth',8);

    midpoint = 0.5*(hinge_L + hinge_R);
    im = insertText(im,midpoint,[num2str(axis_length(i),'%.0f') ' pixels'], ...
        FontSize = 96,TextBoxColor="w");
...
%%
exportgraphics(gcf,"DSCN2849 - Annotated.png","Resolution",600)
