...
run_name1 = 'square_run1';

A0 = zeros(2*(N*(N_modes)-constraint_count),1);
A0 = A0 + 1e-5*rand(size(A0));
...
computational_domain = [0 0.2];         % What is the domain of b to explore
...
for i = 1:length(BP)
    run_name = ['square_run' sprintf('%.0f',1+i)];
    prob = coco_prob();
...
for i = 1:length(UZ)
    run_name = ['square_run' sprintf('%.0f',1+i+length(BP))];
    prob = coco_prob();
...
close all
cd('data');
square_runs = dir('square*');
cd ..

theme1 = struct('special', {{'EP','FP','HB','BP'}});
figure(9899); clf; hold on

for i = 1:length(square_runs)
    coco_plot_bd(theme1, square_runs(i).name, 'x',1,'x',2,'b')
    plot_shape_from_COCO(square_runs(i).name,N,N_modes,adjacency_matrix,points,coeff_matrix,modes_to_skip)
