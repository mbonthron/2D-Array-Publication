...
for i = 1:length(UZ)
    ...
    if data.plot_COCO
        ...
        figure(1); hold on
        scatter3(bcrits(i),x2(i),200,"MarkerFaceColor",'c',"MarkerEdgeColor","k")
        text(bcrits(i),x2(i),num2str(lbl),'HorizontalAlignment','center','FontSize',14,'FontWeight','bold')
    else
        data.A0 = A(:,i)';
        data.b_vector = bcrits(i)*ones(N,1)';
        V_vector = calculate_energy(data);
