...
for v = 1:2
    im_OG = insertShape(im_OG,'Circle',[verts(v,:) verts_radius(v)],'Color','green','linewidth',lw);
end
...
for i = 1
    ...
    quarterpoint = 0.75*(p1) + 0.25*(p2);
    midpoint = 0.5*(p1 + p2);
    threequarterpoint = 0.25*(p1) + 0.75*(p2);
    slope = (p2(2) - p1(2))/(p2(1) - p1(1));
    perpindicular = - 1 / slope;

    perp_dir = [1 perpindicular];
    perp_dir = perp_dir / norm(perp_dir);

    % Plot the x axis
    im_OG = insertShape(im_OG,'Line',[p2  p1],'Color','green','linewidth',lw);

    % Plot the midpoint of the axis
    im_OG = insertShape(im_OG,'Circle',[midpoint 10],'Color','green','linewidth',lw);

    % Plot the perpendicular lines
    end1 = quarterpoint + [perp_length*perp_dir(1) perp_length*perp_dir(2)];
    end2 = quarterpoint - [perp_length*perp_dir(1) perp_length*perp_dir(2)];
    im_OG = insertShape(im_OG,'Line',[quarterpoint  end1],'Color','green','linewidth',lw);
    im_OG = insertShape(im_OG,'Line',[quarterpoint end2],'Color','green','linewidth',lw);

    end1 = midpoint + [perp_length*perp_dir(1) perp_length*perp_dir(2)];
    end2 = midpoint - [perp_length*perp_dir(1) perp_length*perp_dir(2)];
    im_OG = insertShape(im_OG,'Line',[midpoint  end1],'Color','green','linewidth',lw);
    im_OG = insertShape(im_OG,'Line',[midpoint end2],'Color','green','linewidth',lw);


    end1 = threequarterpoint + [perp_length*perp_dir(1) perp_length*perp_dir(2)];
    end2 = threequarterpoint - [perp_length*perp_dir(1) perp_length*perp_dir(2)];
    im_OG = insertShape(im_OG,'Line',[threequarterpoint  end1],'Color','green','linewidth',lw);
    im_OG = insertShape(im_OG,'Line',[threequarterpoint end2],'Color','green','linewidth',lw);
