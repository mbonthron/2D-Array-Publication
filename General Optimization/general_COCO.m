function [data,run_max_E_per_b,bpoints] = general_COCO(data, bpoints, all_branches)
...
computational_domain = [-0.01 max([0,bpoints+.02])];          % What is the domain of b to explore
UZpoints = bpoints;
...
for i = 1:length(UZpoints)
    ...
    elseif all_branches == -1
        ...
        if ~isempty(run_number_max)
            ...
            % Save the A0hatp
            if ~exist("COCO mat files\", 'dir')
                mkdir("COCO mat files\");
            end
