...
if n == 1
    ...
    j = 1;
    dVdt(1) = (1/12)*(t^2)*(j^4).*A(indices(j)) - (j^2).*(deltaL - 1/2*e*A(indices(1))  - 0.25.*(sum1)).*(A(indices(j))+e);
    
    % Write the equation for all subsequent modes
    for j = 2:N_modes
        dVdt(j) = (1/12)*(t^2)*(j^4).*A(indices(j)) - (j^2).*(deltaL - 1/2*e*A(indices(1))  - 0.25.*(sum1)).*A(indices(j));
    end
else
