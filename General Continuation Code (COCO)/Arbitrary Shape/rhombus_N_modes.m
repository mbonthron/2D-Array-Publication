...
for i = 1:length(BP)
    run_name = ['rhombus_run' sprintf('%.0f',1+i)];
    prob = coco_prob();
...
for i = 1:length(UZ)
    run_name = ['rhombus_run' sprintf('%.0f',1+i+length(BP))];
    prob = coco_prob();
...
zlim([0 0.15]); xlim(0.1*[-1 1]); ylim(0.05*[-1 1])
