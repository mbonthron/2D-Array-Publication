function [disp_dimensional, Rxn_dimensional_return] = snap_through_analytical_P(data)
...
tic
[T_contact,U_contact] = ode45(@(t,x0) in_contact_P(t,x0,eta,b,beta,indentor_speed),linspace(0,T_end,5000),U);
toc
...
for i = 1:1:length(T_contact)
    [Q1,a1cVAL,a1cdotVAL] = in_contact2_P(T_contact(i),U_contact(i,:),eta,b,beta,indentor_speed);
    Rxn(i) = Q1;
    a3(i) = a1cVAL;
    a3dot(i) = a1cdotVAL;
end
...
[T_free,U_free] = ode45(@(t,x0) snap_through_P(t,x0,b,beta),linspace(T_contact(end),T_end,500),U_contact(end,:));
...
%%
figure(4); hold on
plot(disp_dimensional*1000,Rxn_dimensional(1:snap_through_index),"-","LineWidth",3,'DisplayName',"PRL eta = " + eta)
xlabel('Displacement')
ylabel('Force')
legend()
...
save("b = "+data.rise*1000+"mm L =" +data.L*1000 +"mm EE = "+EE/(1e9)+"e9 beta = "+beta+" eta =" + eta + ".mat",'disp_dimensional','Rxn_dimensional','snap_through_index','U_combined_dimensional','T_combined_dimensional')
