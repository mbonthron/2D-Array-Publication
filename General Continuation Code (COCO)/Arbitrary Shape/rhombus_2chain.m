...
run_number = 1;
run_name1 = ['rhombus_2chain_run' sprintf('%.0f',run_number)];
...
initial_parameter_value = [0;0.1*pi];      % Starting values of b and t

active_continuation_parameter = 'b';    % Which parameter do we want to vary
computational_domain = [0 0.5];         % What is the domain of b to explore
UZpoints = [0.35];                       % Values to explicitly call out to be used to plot
...
for i = 1:length(BP)
    run_name = ['rhombus_2chain_run' sprintf('%.0f',run_number)];
    prob = coco_prob();
...
%% ===============
% Continue from the branch points of the sixth run
run_name_start_from = ['rhombus_2chain_run' sprintf('%.0f',6)];
...
% 
% % Continue from the branch points of the fifth run
% run_name_start_from = ['rhombus_2chain_run' sprintf('%.0f',5)];
% BP2 = coco_bd_labs(run_name_start_from, 'BP'); % labels for BP points in run1
% 
% for i = 1:length(BP2)
%     run_name = ['rhombus_2chain_run' sprintf('%.0f',run_number)];
...
close all
cd('data');
rhombus_runs = dir('rhombus_2chain*');
cd ..
...
zlim([0 0.5]); xlim(0.5*[-1 1]); ylim(0.25*[-1 1])


%% Coco Plotting Trouble Shoot
figure(9900); clf; hold on
coco_plot_bd(theme1, 'rhombus_2chain_run1', 'x',1,'x',2,'b')
% coco_plot_bd(theme1, 'rhombus_2chain_run2', 'x',1,'x',2,'b')
% coco_plot_bd(theme1, 'rhombus_2chain_run3', 'x',1,'x',2,'b')
% coco_plot_bd(theme1, 'rhombus_2chain_run4', 'x',1,'x',2,'b')
% coco_plot_bd(theme1, 'rhombus_2chain_run5', 'x',1,'x',2,'b')
coco_plot_bd(theme1, 'rhombus_2chain_run6', 'x',1,'x',2,'b')
axis tight; grid on; view(3)
zlim([0 0.5]); xlim(0.5*[-1 1]); ylim(0.25*[-1 1])
